...
Stores currently open modals in the order they were opened and manages their visual stacking order. When the stack order is changes, the `stackChange` event is emitted.

#### Example

Below is an example implementation of using stack along with the `propStore` plugin to persist the state of open modals across page sessions.
...
Returns the active modal if one exists. Otherwise, returns undefined.
...
Closes a modal using the provided modal ID. It will throw an error if the provided ID does not match an existing modal entry.
...
Replaces currently opened modals with a specific modal. Returns an objected containing the `opened` modal and an array of `closed` modals.

#### Arguments

<ReferenceCard name="id" type="string">
  The ID of the modal to replace currently opened modals.
</ReferenceCard>
...
<ReferenceCard name="exclude" type="string">
  The ID of the modal to exclude from closing modals.
</ReferenceCard>
...
Returns whether or not the modal is required or not.
...
<ReferenceCard name="stackChange" type="event">
  Emitted when a modal stack order changes. This happens when a modal is opened, closed or moved to the top of the stack.
