...
Removes the collection entry from the parent collection. After being removed, the `onDeregisterEntry` lifecycle hook is run. Returns the deregistered entry.
