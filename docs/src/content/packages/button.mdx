---
title: Button
description: "Buttons are a simple component that allow users to take actions."
package: "@vrembem/button"
---

import Icon from '../../components/Icon.vue';
import CodeExample from '../../components/CodeExample.astro';
import DataCard from '../../components/DataCard.astro';

# Button

Buttons are a simple component that allow users to take actions.

```sh
npm install @vrembem/button
```

<CodeExample lang="scss">
  ```scss
  @use "@vrembem/button";
  ```
</CodeExample>

## Basic usage

The most basic implementation of the button component consists of the `button` class that can be applied to `<button>`, `<a role="button">` or `<input>` elements with a type attribute of "button", "submit" or "reset". [Remember to add `role="button"`](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/Roles/button_role) to any none semantic buttons as needed.

<CodeExample>
  <div slot="output" class="level">
    <button class="button">Button</button>
    <a class="button" role="button">Anchor</a>
    <input class="button" type="button" value="Input" />
    <input class="button" type="submit" value="Submit" />
    <input class="button" type="reset" value="Reset" />
  </div>
  ```html
  <button class="button">Button</button>
  <a class="button" role="button">Anchor</a>
  <input class="button" type="button" value="Input" />
  <input class="button" type="submit" value="Submit" />
  <input class="button" type="reset" value="Reset" />
  ```
</CodeExample>

Elements within a button are centered vertically and spaced accordingly using the value set in `$gap` (defaults to `0.5em`). When elements are added inside a button, the button's text should also be wrapped in a `<span>` so that it can be spaced properly.

<CodeExample>
  <div slot="output">
    <button class="button">
      <Icon name="anchor" iconClass="icon_size_sm" />
      <span>Button</span>
      <span class="arrow"></span>
    </button>
  </div>
  ```html
  <button class="button">
    <svg class="icon icon_size_sm">...</svg>
    <span>Button</span>
    <span class="arrow"></span>
  </button>
  ```
</CodeExample>

### Disabled

When disabled using the `disabled` attribute, a button will inherit styles to visually appear noninteractive.

<CodeExample>
  <div slot="output">
    <button class="button" disabled>Button</button>
  </div>
  ```html
  <button class="button" disabled>Button</button>
  ```
</CodeExample>

### Loading

Buttons can also have a loading state by adding the `is-loading` state class. This is useful when a button has an action that has a delayed response in order to give users a visual indicator that their action is being processed.

<CodeExample>
  <div slot="output">
    <button class="button is-loading" disabled>Button</button>
  </div>
  ```html
  <button class="button is-loading" disabled>Button</button>
  ```
</CodeExample>

> Warning: the `is-loading` state class will not work on `<input type="button">` buttons since it uses the `::after` pseudo-element to display the loading spinner.

## button_block_[value]

Gives a button "block" styles so that it spans the full width of its container. Values and class keys are generated using the [`$breakpoints`](#breakpoints) map. Omitting the key value from the modifier (e.g. `button_block`) will apply block styles under all conditions.

<CodeExample>
  <div slot="output">
    <button class="button button_block">Button</button>
  </div>
  ```html
  <button class="button button_block">Button</button>
  ```
</CodeExample>

**Available Variants**

- `button_block`
- `button_block_xs`
- `button_block_sm`
- `button_block_md`
- `button_block_lg`
- `button_block_xl`

## button_color_[value]

Adds styles for changing the look and feel of a button. These are usually done with different background and text colors.

<CodeExample>
  <div slot="output" class="level">
    <button class="button button_color_primary">Button</button>
    <button class="button button_color_secondary">Button</button>
  </div>
  ```html
  <button class="button button_color_primary">Button</button>
  <button class="button button_color_secondary">Button</button>
  ```
</CodeExample>

**Available Variants**

- `button_color_primary`
- `button_color_secondary`

## button_icon

Adds styles that make icon-only buttons have equal padding. Will be square if the icon used also has equal width and height.

<CodeExample>
  <div slot="output">
    <button class="button button_icon">
      <Icon name="anchor" />
    </button>
  </div>
  ```html
  <button class="button button_icon">
    <svg class="icon">...</svg>
  </button>
  ```
</CodeExample>

## button_size_[value]

Adjust the size of a button by increasing or decreasing its padding and font-size.

<CodeExample>
  <div slot="output" class="level">
    <button class="button button_size_lg">Button</button>
    <button class="button">Button</button>
    <button class="button button_size_sm">Button</button>
  </div>
  ```html
  <button class="button button_size_lg">Button</button>
  <button class="button button_size_sm">Button</button>
  ```
</CodeExample>

**Available Variants**

- `button_size_sm`
- `button_size_lg`

## Customization

Buttons are primarily customized using CSS variables but can also be modified using Sass variables.

### CSS Variables

All CSS variables are given the Vrembem vendor prefix `"vb-"`. This can be changed by setting the `$prefix-variable` variable either globally via the `core` module or per component.

<div class="border-top">

  <DataCard 
    lang = "css"
    name="button-size"
    value="button.$size"
    computed="2.5rem"
  />

  <DataCard 
    lang = "css"
    name="button-padding"
    value="button.$padding"
    computed="calc(0.5em - 1px) 1em"
  />

  <DataCard 
    lang = "css"
    name="button-gap"
    value="button.$gap"
    computed="0.5rem"
  />

  <DataCard
    lang = "css"
    name="button-border-width"
    value="button.$border-width"
    computed="1px"
  />

  <DataCard
    lang = "css"
    name="button-border-radius"
    value="button.$border-radius"
    computed="0.25rem"
  />

  <DataCard
    lang = "css"
    name="button-font-size"
    value="button.$font-size"
    computed="1em"
  />

  <DataCard
    lang = "css"
    name="button-font-weight"
    value="button.$font-weight"
    computed="inherit"
  />

  <DataCard
    lang = "css"
    name="button-line-height"
    value="button.$line-height"
    computed="1.625"
  />

  <DataCard
    lang = "css"
    name="button-transition-property"
    value="button.$transition-property"
    computed="background, color, border-color, box-shadow"
  />

  <DataCard
    lang = "css"
    name="button-transition-duration"
    value="button.$transition-duration"
    computed="0.15s"
  />

  <DataCard
    lang = "css"
    name="button-transition-timing-function"
    value="button.$transition-timing-function"
    computed="cubic-bezier(0.4, 0, 0.2, 1)"
  />

  <DataCard
    lang = "css"
    name="button-background"
    value="button.$background"
    computed="transparent"
  />

  <DataCard
    lang = "css"
    name="button-background-hover"
    value="button.$background-hover"
    computed="transparent"
  />

  <DataCard
    lang = "css"
    name="button-background-active"
    value="button.$background-active"
    computed="transparent"
  />

  <DataCard
    lang = "css"
    name="button-foreground"
    value="button.$foreground"
    computed='var(--vb-foreground)'
  />

  <DataCard
    lang = "css"
    name="button-foreground-hover"
    value="button.$foreground-hover"
    computed="null"
  />

  <DataCard
    lang = "css"
    name="button-foreground-active"
    value="button.$foreground-active"
    computed="null"
  />

  <DataCard
    lang = "css"
    name="button-border-color"
    value="button.$border-color"
    computed="var(--vb-border-color-dark)"
  />

  <DataCard
    lang = "css"
    name="button-border-color-hover"
    value="button.$border-color-hover"
    computed="var(--vb-border-color-darker)"
  />

  <DataCard
    lang = "css"
    name="button-border-color-active"
    value="button.$border-color-active"
    computed="undefined"
  />

  <DataCard
    lang = "css"
    name="button-box-shadow"
    value="button.$box-shadow"
    computed="0 0 0 0 rgba(black, 0)"
  />

  <DataCard
    lang = "css"
    name="button-box-shadow-focus"
    value="button.$box-shadow-focus"
    computed="0 0 0 0.2rem rgba(black, 0.15)"
  />

  <DataCard
    lang = "css"
    name="button-loading-size"
    value="button.$loading-size"
    computed="1em"
  />

  <DataCard
    lang = "css"
    name="button-loading-border"
    value="button.$loading-border"
    computed="2px solid"
  />

  <DataCard
    lang = "css"
    name="button-loading-animation-duration"
    value="button.$loading-animation-duration"
    computed="0.6s"
  />

  <DataCard
    lang = "css"
    name="button-loading-animation-timing-function"
    value="button.$loading-animation-timing-function"
    computed="linear"
  />

  <DataCard
    lang = "css"
    name="button-disabled-opacity"
    value="button.$disabled-opacity"
    computed="0.6"
  />

</div>

### SCSS Variables

<div class="border-top">

  <DataCard 
    name="prefix-variable"
    value="core.$prefix-variable"
    computed='"vb-"'
  />

  <DataCard 
    name="prefix-block"
    value="core.$prefix-block"
    computed="null"
  />

  <DataCard 
    name="prefix-element"
    value="core.$prefix-element"
    computed='"__"'
  />

  <DataCard 
    name="prefix-modifier"
    value="core.$prefix-modifier"
    computed='"_"'
  />

  <DataCard 
    name="prefix-modifier-value"
    value="core.$prefix-modifier-value"
    computed='"_"'
  />

  <DataCard 
    name="breakpoints"
    value="core.$breakpoints"
  >
    ```scss
    $breakpoints: (
      "xs": 480px,
      "sm": 620px,
      "md": 760px,
      "lg": 990px,
      "xl": 1380px
    ) !default;
    ```
  </DataCard>

  <DataCard 
    name="transition-property"
    value="background, color, border-color, box-shadow"
    computed="background, color, border-color, box-shadow"
  />

  <DataCard 
    name="transition-duration"
    value="core.$transition-duration-short"
    computed="0.15s"
  />

  <DataCard 
    name="transition-timing-function"
    value="core.$transition-timing-function"
    computed="cubic-bezier(0.4, 0, 0.2, 1)"
  />

  <DataCard 
    name="size"
    value="core.$form-control-size"
    computed="2.5rem"
  />

  <DataCard 
    name="padding"
    value="calc(#{list.nth(core.$padding, 1)} - 1px) list.nth(core.$padding, 2)"
    computed="calc(0.5em - 1px) 1em"
  />

  <DataCard 
    name="gap"
    value="0.5rem"
    computed="0.5rem"
  />

  <DataCard 
    name="border-width"
    value="1px"
    computed="1px"
  />

  <DataCard 
    name="border-radius"
    value="core.$border-radius"
    computed="0.25rem"
  />

  <DataCard 
    name="font-size"
    value="1em"
    computed="1em"
  />

  <DataCard 
    name="font-weight"
    value="inherit"
    computed="inherit"
  />

  <DataCard 
    name="line-height"
    value="core.$line-height"
    computed="1.625"
  />

  <DataCard 
    name="background"
    value="transparent"
    computed="transparent"
  />

  <DataCard 
    name="background-hover"
    value="transparent"
    computed="transparent"
  />

  <DataCard 
    name="background-active"
    value="transparent"
    computed="transparent"
  />

  <DataCard 
    name="foreground"
    value='theme.get("foreground")'
    computed="var(--vb-foreground)"
  />

  <DataCard 
    name="foreground-hover"
    value="null"
    computed="null"
  />

  <DataCard 
    name="foreground-active"
    value="null"
    computed="null"
  />

</div>

```scss
$border-color: theme.get("border-color-dark");
$border-color-hover: theme.get("border-color-darker") !default;
$border-color-active: null !default;

$box-shadow: 0 0 0 0 rgba(black, 0) !default;
$box-shadow-focus: 0 0 0 0.2rem rgba(black, 0.15) !default;

// disabled
$disabled-opacity: 0.6 !default;

// is-loading
$loading-size: 1em !default;
$loading-border: 2px solid !default;
$loading-border-tpl: 1 1 0 0;
$loading-animation-duration: 0.6s !default;
$loading-animation-timing-function: linear !default;

// button_icon
$icon-padding: calc(#{list.nth(core.$padding, 1)} - 1px);
$icon-padding-sm: calc(#{list.nth(core.$padding-sm, 1)} - 1px);
$icon-padding-lg: calc(#{list.nth(core.$padding-lg, 1)} - 1px);

// button_size_sm
$size-sm: core.$form-control-size-sm !default;
$size-sm-padding: calc(#{list.nth(core.$padding-sm, 1)} - 1px) list.nth(core.$padding-sm, 2) !default;
$size-sm-font-size: core.$font-size-sm !default;
$size-sm-line-height: core.$line-height-sm !default;

// button_size_lg
$size-lg: core.$form-control-size-lg !default;
$size-lg-padding: calc(#{list.nth(core.$padding-lg, 1)} - 1px) list.nth(core.$padding-lg, 2) !default;
$size-lg-font-size: core.$font-size-lg !default;
$size-lg-line-height: core.$line-height-lg !default;
```

### SCSS Mixins

```scss
@mixin button-variant($foreground, $background)
```
