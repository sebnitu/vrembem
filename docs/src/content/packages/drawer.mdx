...
Closes a drawer using the provided drawer ID. Will throw an error if the provided ID does not match an existing drawer entry.
...
Toggles a drawer opened or closed depending on its current state using the provided drawer ID. Will throw an error if the provided ID does not match an existing drawer entry.
...
Stores the current mode of the drawer.

### entry.state

<ReferenceBar type="property" returns="string" />

Stores the current state of the drawer. Possible states:
...
Stores the inline state of the drawer.
