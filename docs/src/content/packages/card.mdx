...
### Background and screen elements
...
Below is a reference to all CSS variables that are defined or referenced by the `card` component along with their fallback values. Some variables are only referenced and not defined, allowing you to easily customize styles by setting your own values as needed.

<ReferenceTable id="cssvar-card" valueLabel="Fallback" data={{
  "--vb-card-background": {
