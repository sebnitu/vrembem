...
<ReferenceCard ref="config" name="utility-classes" type="map" defaults='("separator": "sep")'>
  A map containing module names as keys and their preferred class selectors as values. For example, to use the class `.radius` instead of `.border-radius`, you could set the following:
