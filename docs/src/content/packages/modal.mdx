...
To take full advantage of modal's accessibility features, it's recommended to set the `selectorInert` option to all elements that are outside the modals. All elements that match the `selectorInert` selector will be given the `inert` attribute, as well as `aria-hidden="true"` when a modal is opened. If modal markup are rendered throughout your document, use the [`teleport`](/packages/core/teleport) plugin to consolidate all modals in the DOM to a single location.
...
Below is an example implementation of using stack along with the [`propStore`](/packages/core/prop-store) plugin to persist the state of open modals across page sessions.
