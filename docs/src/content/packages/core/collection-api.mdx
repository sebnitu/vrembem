...
Creates an instance of a `CollectionEntry` using the provided CSS selector or HTML element. This will also run the `init` method of the collection entry if it exists and fire the `onCreateEntry` event hook. Returns the created collection entry object.

#### Arguments

<ReferenceCard name="query" type="Partial<CollectionConfig>" defaults="string | HTMLElement">
  The query argument specifies a CSS selector or HTML element used to create a new collection entry.
</ReferenceCard>
...
<ReferenceBar type="method" returns="Promise<CollectionEntry | null>" />
