...
Returns the plugin with the specified name from the array, or `null` if no matching plugin is found. If provided with the string `"*"`, the entire plugin array is returned.
...
Adds a plugin to the array. Plugins must have a unique name. Added plugin objects will have their configuration built by merging the values of the `config`, collection `presets`, and `options` properties. Lastly, the `setup` method of the plugin object is run.
