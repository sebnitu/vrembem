...
Collections are configured using an options object, which can be provided during instantiation, via the mount method, or by calling `updateConfig`. The following outlines the structure of this options object:
...
<CodeExample lang="ts">
```ts
type CollectionEntryConstructor<
  TParent extends Collection, 
