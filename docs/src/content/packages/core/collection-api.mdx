...
Adds a plugin to the array. Plugins must have a unique name. Added plugin objects will have their configuration built by merging the values of the `config`, collection `presets`, and `options` properties. Lastely, the `setup` method of the plugin object is run.
