...
Collections implement `PluginArray` under the `plugins` property to manage and interact with plugins that extend or modify collection behavior. It allows you to `add`, `remove`, and `get` plugins by name. Plugins can hook into collection lifecycle events, enabling advanced customization and functionality for collection instances.

### plugins.get

<ReferenceBar type="method" returns="Plugin | null" />

Returns the plugin with the specified name from the array, or `null` if no matching plugin is found.
...
Adds one or more plugins to the array. If a plugin with the same name already exists, it is replaced; otherwise, the new plugin is added. Plugin configuration is built and validated before being added.

#### Arguments

<ReferenceCard name="plugin" type="Plugin | Plugin[]">
...
Removes the plugin with the specified name from the array, if it exists.
