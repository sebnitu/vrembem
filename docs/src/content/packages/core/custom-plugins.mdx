...
Plugins can be installed to a colleciton by passing their object or a function that returns their object to the `plugins` array option.
...
const myCollection = new Collection({
  plugins: [
    ...
    anotherPlugin(options), // Should return a plugin object with options
    { 
      name: "simplePlugin",
      config: { ... },
      setup() {
...
A plugin is comprised of properties, lifecycle hooks and methods. The only requirment for a plugin is that it has a unique name property. A plugins configuration object is built when added to a collection by merging the values of the `config`, collection presets (if they exist) and `options` properties.
...
It is common convention to author Vrembem plugins as a factory function that returns the actual plugin object. The function can accept options which allows users to customize the behavior of the plugin.
