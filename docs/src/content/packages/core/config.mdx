...
Core and other components that define configuration properties can also be set using the Sass `with` clause and setting the `$config` map:

<CodeExample label="app.scss">
  ```scss
  // Set values to config using @use ... with
  ...
  // Values set in $config should not be prefixed by the component name
  ...
  // Core configurations are not prefixed
  @debug core.config-get("prefix-block"); // => "sn-"

  // Component configurations are prefixed with their name
  @debug core.config-get("base-output"); // => false
  ```
</CodeExample>

<div class="notice notice_color_primary">
  Component specific configurations are prefixed with their component name. Core is the only exception.
</div>
