...
To set configuration options via the Sass `@use ... with` clause define the `$config` map. Values set using `with ($config)` are stored prefixed with the component's name. The core component is the only exception to this.
...
<CodeExample label="app.scss">
  ...
  // Core config options are not prefixed
  @debug core.config-get("prefix-block"); // => "sn-"

  // Component config options are prefixed with their name
