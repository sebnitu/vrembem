...
Apply a configuration map prefixing all keys with the provided prefix value. The provided configuration map must match the signature list or default map. This mixin is typically used by components to allow providing default values using the Sass `with` directive.
...
Consider the following setup where we define an empty `$config` map (with the `!default` flag) and a `$default` map containing the configurations we want to expose and provide defaults for.
...
When we use this module, we can define the configuration using the Sass `with` directive:
