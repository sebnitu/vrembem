...
Each `CollectionEntry` implements the `ConfigManager` module to manage its configuration. Upon creation, the entry's `ConfigManager` is initialized under the `entry.config` property where all its APIs are made available. Two config sources are defined by default:
...
Entries can have additional config sources defined using `config.addConfigSource` where you pass a `sourceKey` and config object as arguments. The order of presidence can be updated using `config.setSourceOrder` and providing an array of `sourceKey` values to set a specific order. Lastly, you can use plugins to expand the configurability of entries:
