...
Players can now take turns selecting cells, but we still need to track when a player wins or the game ends in a draw. To help manage these states, we'll add custom events using the collection's event system.
...
Lifecycle hooks also fire their own custom events. This enables other parts of your project to react to key moments in a component's lifecycle, promotes loose coupling between modules, and makes it easy to extend or customize behavior without modifying core logic.

Lets use events to allow entries to communicate with their parent collection. We'll do this by emitting an `afterTurn` event in the `TicTacToeCell` toggle method that we then listen for in the collection's `afterMount` lifecycle hook. This allows us to move the `switchPlayer` call from the entries logic into the parent collection itself.
