...
Players can now take turns selecting cells, but we still need to track when a player wins or the game ends in a draw. To handle these states, we'll add custom events using the collection's event system.
