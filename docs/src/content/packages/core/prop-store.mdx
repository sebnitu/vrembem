...
By default, `propStore` will watch the `state` property of your entries for changes. Whenever the watched property changes, the new value is stored in local storage and the provided `onChange` function is fired.
...
In this example, we'll create a new collection for a group of input fields where we want to persist their input values. First, we'll set up `propStore` to watch the `value` property. Whenever the `value` property changes, we'll use the `onChange` callback to update the value of our input fields.
...
We can also provide an `afterMount` lifecycle hook to `propStore` where we can add an event listener to collection entry inputs. Here we just want to update our `entry.value` property whenever an input value changes.
...
Lastly, we need some input markup with our required selector to finish up the example. Each input now uses `propStore` to persist their input values across page reloads.
...
There are two common use cases for `propStore` in both Modal and Drawer components. For Modals, you may want to persist modal state across page load and refresh. Here is an example to accomplish this while also maintaining nested modal support:
...
const modal = new Modal({
  ...
  plugins: [
    propStore({
      // Disable automatic local storage
      condition: false,
      async afterMount({ plugin, parent, entry }) {
        ...
        parent.on("stackChange", () => {
          // Store the current stack value when it changes
