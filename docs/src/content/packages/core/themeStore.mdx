...
An object to store the values of `themeStore`'s configurations. These include:
...
- `themes (string[])` - An array of all available themes.
- `onInit: () => void` - The function to run when themeStore is initialization.
- `onChange: () => void` - The function to run when the theme changes.
