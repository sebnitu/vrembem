...
Once loaded, the content package provides normalization and default styles for HTML elements along with a number of element classes. To style HTML elements, use their corresponding classes or apply the `content` class to style an entire block of content.
...
<ReferenceBar selector=".content" hash={{
...
The content module provides the `.content` CSS class as a way to apply many content module styles to HTML elements directly. This is most commonly used to style a block of HTML content like an article or blog post.

<CodeExample lang="html">
<div slot="output" class="content spacing">
  <h1>Heading</h1>
...
<ReferenceCard ref="config" name="content-selectors" type="map">
