---
title: Content
description: "A content wrapper that provides typographic styles and base formatting for common HTML elements."
package: "@vrembem/content"
category: modules
---
...
import data from "@/data/tokens-content.json";
...
Content is built as a Sass module. Sass options are used to control CSS output, while CSS variables control the styling of the component. Below are the available configuration options.

### Sass

Content comes with the following Sass configuration options that can be set using either the [config module](/packages/core/config#using-config-module) or [Sass `with` clause](/packages/core/config#using-with-clause) methods.
...
Content uses CSS variables in a reference and fallback pattern. Variables are referenced in the component's styles but are not directly defined; instead, they are provided with sensible fallbacks.

<ReferenceTable id="tokens-content" data={data} expandable={true} valueLabel="Fallback" />
...
<ReferenceCard ref="config" name="content-output" type="boolean" defaults="true">
...
<ReferenceCard ref="config" name="content-output-exceptions" type="list" defaults="()">
...
Once loaded, the content package provides normalization and default styles for HTML elements along with a number of element classes. To style HTML elements, use their corresponding classes or apply the `type` class to style an entire block of content.

<CodeExample lang="html">
```html
<!-- Using content element modules -->
<h1 class="h1">...</h1>
<p>...</p>
<ul class="list">
  <li>...</li>
...
<ReferenceCard ref="config" name="content-classes" type="map" defaults='("separator": "sep")'>
...
It's also possible to provide a prefix to all content classes using the `content-prefix` config option.

<ReferenceCard ref="config" name="content-prefix" type="map" defaults='""'>
...
Below is a list of each content module, their class selectors, CSS variables, and intended usage. Modules that reference CSS variables are provided with sensible fallbacks.

### headings

<ReferenceBar selector=".h1 - .h6" hash={{
  target: "reference-table-tokens-content",
  event: "filter",
  detail: {
    value: "--vb-headings"
  }
...
<ReferenceCard ref="config" name="content-headings" type="map">
...
The type module provides the `.content` CSS class as a way to apply many content module styles to HTML elements directly. This is most commonly used to style a block of HTML content like an article or blog post.
...
The type module only applies styles to its direct children using the `>` child combinator. This is to allow nesting of other components within a type block and avoid style conflicts. Module specific type selectors are controlled using the `content-type-selectors` variable map.

<ReferenceCard ref="config" name="content-type-selectors" type="map">
  Map modules to type selectors. This configuration is used when a module runs the `content.selector()` function. The keys of the map should represent a content module name and it's value the selector to be used within the `.type` context.

```scss
@include config.set("content-type-selectors", (
  "h1": "> h1",
  "h2": "> h2",
  "h3": "> h3",
  // ...
