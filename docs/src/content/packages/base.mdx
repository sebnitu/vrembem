...
Base modules each have their own output styles. These are all output by default when the base package is loaded. There are two ways to control which modules styles are output using the `$output` variable and `$output-exceptions` list.
...
You can disable CSS output using the `$output` and `$output-[module]` variables. All base modules have a corresponding output variable that inherits it's value from `$output`.
