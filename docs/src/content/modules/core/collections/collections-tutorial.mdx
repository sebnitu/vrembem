...
description: "Collections are used to create the scaffolding and shared systems that all Vrembem JavaScript components inherit. These systems include configuration, lifecycle hooks, custom events, and plugins."
order: 1
---
...
<div slot="output" id="ttt-1" class="tictactoe-wrapper">
  <table class="tictactoe">
    <tr>
      <td id="cell-A1" class="tictactoe__cell" data-cell="A1" role="button"></td>
      <td id="cell-B1" class="tictactoe__cell" data-cell="B1" role="button"></td>
      <td id="cell-C1" class="tictactoe__cell" data-cell="C1" role="button"></td>
    </tr>
    <tr>
      <td id="cell-A2" class="tictactoe__cell" data-cell="A2" role="button"></td>
      <td id="cell-B2" class="tictactoe__cell" data-cell="B2" role="button"></td>
      <td id="cell-C2" class="tictactoe__cell" data-cell="C2" role="button"></td>
    </tr>
    <tr>
      <td id="cell-A3" class="tictactoe__cell" data-cell="A3" role="button"></td>
      <td id="cell-B3" class="tictactoe__cell" data-cell="B3" role="button"></td>
      <td id="cell-C3" class="tictactoe__cell" data-cell="C3" role="button"></td>
    </tr>
  </table>
...
@include tokens.set(
  "tictactoe",
  (
    "background-winner": tokens.get("primary-background"),
    "foreground-winner": tokens.get("primary-foreground"),
    "border": 0.25em solid tokens.get("neutral-background"),
    "box-shadow-color": tokens.get("background")
  )
);
...
Before diving into our example component, lets go over how configurations are handled within collections and their entries. Each `Collection` holds a config object, which contains global configurations that are accessible to all entries in that collection. Options can be passed to a collection's config either on instantiation or manually using the `updateConfig` method.
...
// Update the config object via `updateConfig` method
myCollection.updateConfig({
  selector: "CSS_SELECTOR"
});
```
</CodeExample>

Each `CollectionEntry` implements the `ConfigManager` module to manage its configuration. Upon creation, the entry's `ConfigManager` is instantiated under the `entry.config` property where all its APIs are made available. Two config sources are defined by default:

- The parent collection's config (as the `"parent"` source).
- Entry-specific options (as the `"entry"` source).

The `entry.config.get` method resolves configuration values by checking the most specific source first (entry), then falling back to the parent collection config if not found. This layered approach ensures that:
...
To provide an entry with a custom configuration object, use the `entry.config.set` method:
...
// Apply a config object manually via `config.set` method
myEntry.config.set({
  selector: "CSS_SELECTOR"
});
```
...
Entries can have additional config sources defined using `config.set` when you pass a `sourceKey` string and config object as arguments. The most recently added source takes precedence over previously defined config sources. The order of precedence can be changed using `config.setSourceOrder` which takes an array of `sourceKey` values. Lastly, you can use plugins to expand the configurability of entries:
...
The `proxyEntry` lifecycle hook allows a collection and its plugins to wrap each entry in a JavaScript Proxy. This enables advanced interception of property access and assignment, making it possible to implement reactive features.

The hook receives the entry and its parent collection as context (plugins also receive their own context) and should return a Proxy handler object. The returned handler is used to create a Proxy that wraps the entry before it is registered in the collection.

<CodeExample lang="js">
```js
class MyCollection extends Collection {
  // ...
  proxyEntry({ parent, entry }) {
    return {
      get(target, prop) {
...
For more information on JavaScript Proxies and their usage, please refer to the [MDN developer documentation](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Proxy).

### Basic usage

We can add a `toggle` method to each cell and attach a click event listener using the `onRegisterEntry` lifecycle hook. Entries also have access to their parent collection via the `this.parent` alias, allowing them to interact with collection methods and properties directly.
...
<CodeExample tabs files={[
  { name: 'TicTacToe.js', content: fs.readFileSync(path.resolve('./src/modules/TicTacToe.js'), 'utf-8') },
  { name: 'TicTacToeCell.js', content: fs.readFileSync(path.resolve('./src/modules/TicTacToeCell.js'), 'utf-8') }
]}>
