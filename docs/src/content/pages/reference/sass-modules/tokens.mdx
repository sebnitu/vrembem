...
You can also define primitive palette colors that can be aliased using semantic tokens. This can be used to create themes by changing the semantic color mapping for a specific theme.
...
To create a theme using palette aliasing, import the palette module and remap aliases in the desired theme namespace. The `palette.remap` function will automatically set `"type": "theme"` on the returned palette map.
...
<CodeExample tabs>
  <TabsPanel label="SCSS">
    ...
    @include tokens.set("core", (
      ...
      "ocean": palette.remap(("primary": "blue"))
    ));
