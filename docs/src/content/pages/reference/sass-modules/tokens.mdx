...
When the `core/palette` plugin is loaded, color tokens are generated using the  values set in `palette` and `palette-variants` config options. The palette plugin also extends the `tokens.get()` function when returning a palette token by allowing more precise control over lightness, chroma and alpha channels.
...
To create a theme using palette aliasing, import the palette plugin module and remap aliases in the desired theme namespace. The `palette.remap` function will automatically set `"type": "theme"` on the returned palette map.
