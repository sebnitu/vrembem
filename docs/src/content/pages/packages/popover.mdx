...
When using the [`cssConfig`](/packages/core/css-config) plugin, the following CSS variables can be used to set JavaScript configuration options.
...
When using the [`cssConfig`](/packages/core/css-config) plugin, the event option can be provided using the `--vb-popover-event` CSS variable. This can be done either through your own custom CSS or using the `style` attribute.
...
When using the [`cssConfig`](/packages/core/css-config) plugin, the placement option can be provided using the `--vb-popover-placement` CSS variable. This can be done either through your own custom CSS or using the `style` attribute.
...
Popovers can be anchored to the cursor element by setting the `followCursor` config option to `true`. When using the [`cssConfig`](/packages/core/css-config) plugin, the followCursor option can be provided using the `--vb-popover-follow-cursor` CSS variable.
